...
document.addEventListener('DOMContentLoaded',setupDataTransferPanel);
...
function renderSummitLedger(){
 ...
 grid.innerHTML=SUMMIT_GROUP_ORDER.map(group=>{
  const rows=filtered.filter(p=>p.status===group.id);
...
document.addEventListener('DOMContentLoaded',setupConvertedHandlers);
...
document.addEventListener('DOMContentLoaded',populateFutureWeatherOptions);
...
document.getElementById('routeIntelDisclosure')?.addEventListener('toggle',event=>{
 if(event.currentTarget.open)renderRouteIntelligence();
});
